...
var LOCK_FILES = ["yarn.lock", "**/yarn.lock"];
var PKG_FILES = ["package.json", "**/package.json"];
var DOCKER_FILES = ["Dockerfile", "**/Dockerfile", "Dockerfile.*", "**/Dockerfile.*", "*.dockerfile", "**/*.dockerfile"];
var spec = {
  "id": "yarn",
  "displayName": "Yarn",
...
async function findDockerMissingPatches(rule, dockerfile, allPaths, repoPath) {
  ...
  let workdir = "/";
  let copies = [];
  const lockfiles = /* @__PURE__ */ new Map();
  const detections = [];
  for (const instruction of instructions) {
    ...
    const missing = lock.patches.filter((patch) => !copyDeliversPatch(copies, patch, posix.join(workdir, ".yarn/patches", basename2(patch))));
    if (missing.length === 0) continue;
    const line = eligibleLine(dockerfile, instruction.line, instruction.endLine) ?? eligibleLine(dockerfile, lock.line, lock.endLine) ?? copies.map((copy) => eligibleLine(dockerfile, copy.line, copy.endLine)).find((candidate) => candidate !== void 0);
    ...
    detections.push({
      ...
      label: `yarn install cannot access ${missing.length === 1 ? basename2(missing[0] ?? "patch") : `${missing.length} referenced patches`}`,
...
function parseDockerCopies(value, workdir, line, endLine) {
  ...
  return sources.map((source) => ({
    ...
    target: posix.normalize(destinationIsDirectory ? posix.join(targetBase, basename2(repoPathFromDocker(source))) : targetBase),
    external,
    line,
    endLine
  }));
...
function localPatchPaths(lockSource, lockfile) {
  ...
  for (const match of lockSource.matchAll(pattern)) {
    const relative3 = decodeURIComponent(match[1] ?? "");
    if (!relative3) continue;
    paths.add(posix.normalize(posix.join(dirname2(lockfile), relative3)));
  }
  return [...paths].sort();
}
